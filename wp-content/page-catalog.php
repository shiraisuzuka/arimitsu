<?php get_header(); ?>

<div class="p-lower-mv">
  <div class="l-section-inner">
    <?php breadcrumb(); ?>
    <hgroup class="p-lower-mv-title">
      <h1>カタログダウンロード</h1>
      <p>catalog</p>
    </hgroup>
  </div>
</div>

<section class="p-catalog l-section">
  <div class="l-section-inner">
    <div class="p-catalog-tab-panel js-tab-panel">
      <ul class="p-catalog-tab c-tab">
        <li class="tab tab-1 is-active">全て</li>
        <li class="tab tab-2">農業機械</li>
        <li class="tab tab-3">産業機械</li>
      </ul>
      <div class="p-catalog-panel">
        <div class="panel tab-1 is-show">
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">ピックアップ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_14.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>自走式動噴<br>セット動墳</h3>
                <p>更新日：2024/10/08</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/54655/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_13.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハイワイヤー<br>オートランナー</h3>
                <p>更新日：2019/01/01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19602/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_09.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>移動式温水<br>高圧洗浄機</h3>
                <p>更新日：2019/01/01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/21107/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_10.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>移動式温水<br>高圧洗浄機</h3>
                <p>更新日：2024/09/12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/25667/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">カタログ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_01.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>総合カタログ</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-66-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19417/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_02.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>静電ノズル</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-68-24.11</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19437/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_03.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハウススプレー</h3>
                <p>更新日：2025/01/06</p>
                <p>コード：K・G-69-24.12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/36289/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_04.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハイクロブーム<br>スプレーヤ</h3>
                <p>更新日：2020/10/29</p>
                <p>コード：K・G-61-20.10★</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19409/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_06.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>土壌消毒機</h3>
                <p>更新日：2023/08/29</p>
                <p>コード：K・G-42G-23.08</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19412/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_07.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>オートランナー</h3>
                <p>更新日：2024/05/31</p>
                <p>コード：K・G-64-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/26593/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_08.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>洗浄機</h3>
                <p>更新日：2024/04/02</p>
                <p>コード：K・G-34M-24.04</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19419/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_09.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>動力散布機</h3>
                <p>更新日：2025/02/18</p>
                <p>コード：K・G-47I-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19411/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_10.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>背負動噴</h3>
                <p>更新日：2025/02/18</p>
                <p>コード：K・G-53L-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/34124/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_11.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>高圧動噴</h3>
                <p>更新日：2025/02/12</p>
                <p>コード：K・G-23I-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19423/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_01.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>総合カタログ</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-66-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/51166/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_02.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>モーター</h3>
                <p>更新日：2024/10/22</p>
                <p>コード：K・S-42J-24.10</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19436/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_03.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>エンジン</h3>
                <p>更新日：2024/05/07</p>
                <p>コード：K・S-43G-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19435/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_04.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>温水</h3>
                <p>更新日：2025/01/20</p>
                <p>コード：K・S-32O-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19434/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_05.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>フォームクリーニング</h3>
                <p>更新日：2023/11/07</p>
                <p>コード：K・S-33H-23.11</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19424/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_06.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>細霧システム</h3>
                <p>更新日：2024/08/11</p>
                <p>コード：なし</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19439/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_07.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>洗浄装置</h3>
                <p>更新日：2024/04/18</p>
                <p>コード：K・S-31M-24.04</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19420/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_08.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>部品洗浄機</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・S-37P-24.12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19421/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_11.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>食品工場へのご提案</h3>
                <p>更新日：2024/05/13</p>
                <p>コード：K・S-44H-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19610/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
        </div>
        <div class="panel tab-2">
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">ピックアップ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_14.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>自走式動噴<br>セット動墳</h3>
                <p>更新日：2024/10/08</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/54655/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_13.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハイワイヤー<br>オートランナー</h3>
                <p>更新日：2019/01/01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19602/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">カタログ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_01.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>総合カタログ</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-66-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19417/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_02.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>静電ノズル</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-68-24.11</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19437/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_03.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハウススプレー</h3>
                <p>更新日：2025/01/06</p>
                <p>コード：K・G-69-24.12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/36289/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_04.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>ハイクロブーム<br>スプレーヤ</h3>
                <p>更新日：2020/10/29</p>
                <p>コード：K・G-61-20.10★</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19409/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_06.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>土壌消毒機</h3>
                <p>更新日：2023/08/29</p>
                <p>コード：K・G-42G-23.08</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19412/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_07.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>オートランナー</h3>
                <p>更新日：2024/05/31</p>
                <p>コード：K・G-64-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/26593/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_08.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>洗浄機</h3>
                <p>更新日：2024/04/02</p>
                <p>コード：K・G-34M-24.04</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19419/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_09.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>動力散布機</h3>
                <p>更新日：2025/02/18</p>
                <p>コード：K・G-47I-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19411/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_10.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>背負動噴</h3>
                <p>更新日：2025/02/18</p>
                <p>コード：K・G-53L-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/34124/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img01_11.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>高圧動噴</h3>
                <p>更新日：2025/02/12</p>
                <p>コード：K・G-23I-25.02</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19423/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
        </div>
        <div class="panel tab-3">
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">ピックアップ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_09.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>移動式温水<br>高圧洗浄機</h3>
                <p>更新日：2019/01/01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/21107/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_10.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>移動式温水<br>高圧洗浄機</h3>
                <p>更新日：2024/09/12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/25667/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
          <div class="p-catalog-list-row">
            <h2 class="p-catalog-list-title">カタログ</h2>
            <ul class="p-catalog-list">
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_01.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>総合カタログ</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・G-66-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/51166/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_02.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>モーター</h3>
                <p>更新日：2024/10/22</p>
                <p>コード：K・S-42J-24.10</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19436/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_03.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>エンジン</h3>
                <p>更新日：2024/05/07</p>
                <p>コード：K・S-43G-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19435/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_04.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>温水</h3>
                <p>更新日：2025/01/20</p>
                <p>コード：K・S-32O-25.01</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19434/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_05.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>フォームクリーニング</h3>
                <p>更新日：2023/11/07</p>
                <p>コード：K・S-33H-23.11</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19424/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_06.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>細霧システム</h3>
                <p>更新日：2024/08/11</p>
                <p>コード：なし</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19439/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_07.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>洗浄装置</h3>
                <p>更新日：2024/04/18</p>
                <p>コード：K・S-31M-24.04</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19420/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_08.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>部品洗浄機</h3>
                <p>更新日：2024/12/27</p>
                <p>コード：K・S-37P-24.12</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19421/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
              <li>
                <figure><img src="<?php img_path(); ?>/catalog/img02_11.jpg" alt="" loading="lazy" width="198" height="280"></figure>
                <h3>食品工場へのご提案</h3>
                <p>更新日：2024/05/13</p>
                <p>コード：K・S-44H-24.05</p>
                <a href="https://ebook.wisebook4.jp/html/arimitsu/19610/#1" target="_blank" class="c-btn">ダウンロード</a>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>

<?php get_template_part('parts/cta'); ?>

<?php get_footer(); ?>